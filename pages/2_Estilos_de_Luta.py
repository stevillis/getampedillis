...
class StyleTournamentApp:
    ...
    def _render_sidebar(self):
        with st.sidebar:
            ...
            styles_df["Icon"] = styles_df["Name"].apply(
                lambda x: (
                    f"https://github.com/stevillis/tournament_image_creator/blob/master/data/styles/{x}.png?raw=true"
                )
            )
            st.data_editor(
                styles_df,
    ...
    def _render_tournament_section(self):
        ...
        with st.container(border=True):
            st.info("Associe os estilos de luta manualmente para cada jogador listado.")
            col_player, col_styles = st.columns([1, 2])
            ...
            if st.button(
                "➕ Adicionar seleção ao campo de estilos", use_container_width=True
            ):
        ...
        st.markdown("#### Dados do Torneio de Estilos")
        _ = st.text_area(
            label="Insira os dados gerados abaixo",
            height=200,
            placeholder="Exemplo:\njogador1, estilo1, estilo2\njogador2, estilo1, estilo2",
            key="style_tournament_data_input",
        )

        if st.button(
            label="🖼️ Criar Imagem do Torneio de Estilos",
            key="create_tournament_style_image",
            use_container_width=True,
        ):
            if len(st.session_state.style_tournament_data_input) == 0:
                st.error("Por favor, insira os dados do torneio de estilos primeiro!")
                return

            players_data = self.validator.validate(
                st.session_state.style_tournament_data_input,
                error_message="Formato inválido. Cada linha deve conter o nome do jogador e pelo menos um estilo.",
            )
    ...
    def _render_team_section(self):
        ...
        with st.container(border=True):
            ...
            with col_btn:
                ...
                if st.button(
                    ...
                    if not selected_team_style_players:
                        ...
                        if "team_style_tournament_data_input" not in st.session_state:
            ...
            if st.button(
                label="🖼️ Gerar Imagens dos Times de Estilos",
                key="create_team_style_images",
                use_container_width=True,
            ):
                ...
                for (
                    line
                ) in st.session_state.team_style_tournament_data_input.splitlines():
                ...
                if "player_styles_data" not in st.session_state:
                    st.error(
                        "Crie a Imagem de Estilos (etapa anterior) primeiro para poder formar times!"
                    )
                    return
...
if __name__ == "__main__":
    st.set_page_config(page_title="Estilos de Luta", page_icon="💪", layout="wide")

    apply_custom_theme()

    st.title("💪 Estilos de Luta")
    st.info(
        "**Como gerar imagens com Estilos:**\n"
        "1. Associe os jogadores aos seus respectivos Estilos na seção de Torneio.\n"
        "2. Gere a imagem do torneio para registrar as escolhas.\n"
        "3. Vá para a Formação de Times, agrupe os jogadores em times e gere as imagens finais!\n"
        "4. Com as imagens geradas, você pode usá-las na **Roleta do Dedé** ou no **Draft Amped**."
    )
    st.markdown("---")
